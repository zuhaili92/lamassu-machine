...
StateMachine.create({
  ...
  events: [
    ...
    { name: 'setCurrentCount', from: 'Disable', to: 'SetCurrentCount' },
    { name: 'getCurrentCount', from: 'Disable', to: 'GetCurrentCount' },
    { name: 'paying', from: ['Disable', 'Paying'], to: 'Paying' },
    { name: 'payStay', from: ['Paying', 'PayStay'], to: 'PayStay' },
    { name: 'payValid', from: ['PayStay', 'PayValid'], to: 'PayValid' }
  ]
})
...
Id003Fsm.prototype.onPayStay = function (event, from) {
  this.emit('billStay')
}

Id003Fsm.prototype.onPayValid = function (event, from) {
  this.emit('billPaid')
  this._dispatch('ack')
}
