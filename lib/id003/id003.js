...
Id003.prototype._run = function _run (cb) {
  ...
  this.id003Fsm.on('billStay', function () {
    self.emit('billStay')
  })

  this.id003Fsm.on('billPaid', function () {
    self.emit('billPaid')
  })
...
Id003.prototype.payout = function payout (bills, cartridgeNumber) {
  const buf = new Buffer(2)
  buf.writeUInt8(bills, 0)
  buf.writeUInt8(cartridgeNumber, 0)
  this._send('payout', buf)
}

Id003.prototype.setRecycleCurrency = function setRecycleCurrency (denom, denomExp, cartridgeNumber) {
  const buf = new Buffer(3)
  buf.writeUInt8(denom, 0)
  buf.writeUInt8(denomExp, 1)
  buf.writeUInt8(cartridgeNumber, 2)

  this._send('setRecycleCurrency', buf)
}

Id003.prototype.getCurrentCount = function getCurrentCount () {
  this._send('getCurrentCount')
}

Id003.prototype.setCurrentCount = function setCurrentCount (count, cartridgeNumber) {
  const buf = new Buffer(3)
  buf.writeUInt16LE(count, 0)
  buf.writeUInt8(cartridgeNumber, 2)
  this._send('setCurrentCount', buf)
}
