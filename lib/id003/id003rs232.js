...
var CMD = {
  ...
  payout: [0xf0, 0x20, 0x4a],
  getCurrentCount: [0xf0, 0xa2],
  setRecycleCurrency: [0xf0, 0x20, 0xd0],
  getRecycleCurrency: [0xf0, 0x20, 0x90],
  setCurrentCount: [0xf0, 0xe2]
}
...
var RSPE = {
  0x90: 'getRecycleCurrency',
  0xd0: 'setRecycleCurrency'
}
...
Id003Rs232.prototype.send = function send (command, data) {
  var codeArr = CMD[command]
  if (!codeArr) throw new Error('Invalid command: ' + command)
  var codes = new Buffer(codeArr)
  var dataLength = data ? data.length : 0
  var length = codes.length + dataLength + 4
  var buf = new Buffer(length)
  buf[0] = SYNC
  buf.writeUInt8(length, 1)
  codes.copy(buf, 2)
  if (data) data.copy(buf, 2 + codes.length)
  var payload = buf.slice(0, length - 2)
  var crc = Crc.compute(payload)
  buf.writeUInt16LE(crc, length - 2)
  // console.log('DEBUG2: %s', prettyHex(buf))
...
Id003Rs232.prototype._parseData = function _parseData (command, rawData) {
  // console.log('DEBUG10')
  // console.log('%s: %s', command, rawData ? rawData.toString('hex') : 'N/A')
  ...
  switch (command) {
    ...
    case 'getRecycleCurrency': return this._getRecycleCurrency(rawData)
    default: return null
  }
}

Id003Rs232.prototype._getRecycleCurrency = function _getRecycleCurrency (rawData) {
