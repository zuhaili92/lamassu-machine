...
function configureMode (width, height, cam) {
  const matchFormat = f => f.width === width && f.height === height && f.formatName === 'MJPG'
  const format = R.find(matchFormat, cam.formats)

  if (!format) throw new Error("Camera doesn't support resolution")
...
function scan (mode, resultCallback, captureCallback) {
  ...
  configureMode(width, height, cam)
  ...
  function capture () {
    ...
    cam.capture(function (success) {
      ...
      captureCallback(frame, greyscale.data, width, height, function (err, result) {
        if (err || result) {
          clearInterval(handle)
          cam.stop()
...
function scanQR (callback) {
  scan('qr', callback, function (frame, greyscale, width, height, _callback) {
...
function scanPDF417 (callback) {
  scan('photoId', callback, function (frame, greyscale, width, height, _callback) {
